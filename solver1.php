...
case "prowadzacy":
    ...
    //$tabela=$metryka_[1];
    $tabela="ocena_pracy_nauczyciela";
   // echo "SELECT prowadzacy.imie,prowadzacy.nazwisko,prowadzacy.tytul,prowadzacy.id"
    ...
    $mozliwe_odpowiedzi=mysqli_query($kon,"SELECT prowadzacy.imie,prowadzacy.nazwisko,prowadzacy.tytul,prowadzacy.id"
            . " from `prowadzacy` INNER JOIN `ocena_pracy_nauczyciela` ON ocena_pracy_nauczyciela.id_prowadzacy=prowadzacy.id  WHERE prowadzacy.rok_akademicki LIKE '%$_POST[rok_akademicki]' GROUP BY prowadzacy.nazwisko,prowadzacy.imie ORDER BY prowadzacy.nazwisko");
    break;
}
$ile=mysqli_num_rows($mozliwe_odpowiedzi);
    ...
    do {
        ...
        $options[]="<option value=\"$wariant[0]\">$wariant[0]</option>";
                ...
                break;
                case "prowadzacy":
                    $ileankiet=mysqli_num_rows(mysqli_query($kon,"SELECT prowadzacy.id FROM `prowadzacy` INNER JOIN `ocena_pracy_nauczyciela`"
                            . "ON prowadzacy.id=ocena_pracy_nauczyciela.id_prowadzacy WHERE `imie`='$wariant[0]'"
                            . "AND `nazwisko`='$wariant[1]' AND `tytul`='$wariant[2]' AND `rok_akademicki`"
                            . " LIKE '%$_POST[rok_akademicki]'"));
                  $options[]="<option value=\"$wariant[3]\">$wariant[0] $wariant[1] $wariant[2] - $ileankiet ankiet</option>";   
                    break;
        }
        $n=$n+1;
    }
...
<div class="card-body">
 ...
 switch($_POST['element']){
     default:
         ...
         break;
         case "prowadzacy":
             echo "<em> Prowadzący zajęcia</em> z ankiety $metryka_[1]";
             break;
 }
 
 ?></h5><hr/>
<form method="post" action="main.php?mode=solver1-1">
