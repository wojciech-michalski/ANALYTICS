...
$queryA="SELECT * FROM `karta_obciazen` WHERE (`kierunek` LIKE 'Zarządanie%' "
        . "OR `kierunek`='Zdrowie Publiczne' OR `kierunek` LIKE 'Mechanika%' OR `kierunek`LIKE 'Ochrona %'"
        . " OR `kierunek`='Informatyka') "
...
$queryB="SELECT * FROM `karta_obciazen` WHERE (`kierunek` LIKE 'Management%' OR `kierunek`LIKE 'Mechnanics%' "
        .  "OR `kierunek` LIKE 'Environment%' OR `kierunek` LIKE 'Public health%' "
        . "OR `kierunek` LIKE 'Computer Engineering%') AND `semestr_ZL`='$_POST[semestr_ZL]' "
        . " AND `rok_akademicki`='$_POST[rok_akademicki]' AND `wykladowca_imie`='$wykladowca[1]' "
    ...
    <li class="nav-item">
        <a class="nav-link" data-toggle="tab" href="#panel2" role="tab">Studia w języku angielskim</a>
         ...
         <div class="card-header text-center" >
             ...
             <span id="toggleprint"  style="margin-left:20%; display:none;"> <button class="btn btn-default" onclick="generatePDFKO();"><i class="fas fa-file-pdf"  ></i> do PDF</button></span>
...
<div class="card-body">
    ...
    <?php if(mysqli_num_rows(mysqli_query($kon,"SELECT `id` FROM `karty_obciazen_w` WHERE"
        ...
        . "`rok_akademicki`='$_POST[rok_akademicki]' AND `semestr_ZL`='$_POST[semestr_ZL]' AND "
            . "(`symbol_studiow` LIKE '%Zarządzanie%' OR `symbol_studiow` LIKE '%Mechanika%'"
        . "OR `symbol_studiow` LIKE '%Zdrowie Publiczne%' OR `symbol_studiow` LIKE '%Ochrona%' "
            . "OR `symbol_studiow` LIKE '%Informatyka%')"))==0) {
        ?>
 ...
 <?php $obciazenia=mysqli_query($kon,$queryA);?>
    ...
    <h5 class="text-center">WYDZIAŁ INŻYNIERII I ZARZĄDZANIA WSEiZ </h5>
...
?></p>
    ...
    <?php 
        ...
        . " AND `rok_akademicki`='$_POST[rok_akademicki]' AND `wykladowca_imie`='$wykladowca[1]' "
        . "AND `wykladowca_nazwisko`='$wykladowca[0]' AND `WYDZIAL`='2' AND `jezyk`='PL' ";
        ?>
             <script>
...
<div class="card-body">
    ...
    <?php if(mysqli_num_rows(mysqli_query($kon,"SELECT `id` FROM `karty_obciazen_w` WHERE"
        . " `wykladowca_imie`='$wykladowca[1]' AND `wykladowca_nazwisko`='$wykladowca[0]' AND "
            . "(`symbol_studiow` LIKE '%Management%' OR `symbol_studiow` LIKE '%Public health%' "
        .  "OR `symbol_studiow` LIKE '%Environmental%' OR `symbol_studiow` LIKE '%Mechanics%' "
            . "OR `symbol_studiow` LIKE '%Computer Engineering%') AND "
...
?></p>
   ...
   <?php 
        ...
        . " AND `rok_akademicki`='$_POST[rok_akademicki]' AND `wykladowca_imie`='$wykladowca[1]' "
        . "AND `wykladowca_nazwisko`='$wykladowca[0]' AND `WYDZIAL`='2' AND `jezyk`='EN' ";
