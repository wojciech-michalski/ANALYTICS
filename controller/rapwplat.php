...
$przynquery_="SELECT `id_kierunek`,`id_typ`,`id_rodzaj`,`kierunek`,`forma`,`stopien`,`tytul` FROM `mapowanie` WHERE (`kierunek`='$kierunki') AND (`forma`='$rodzaje') AND (`stopien`='$typp') AND (`tytul`='$tyt')";
...
do {
    ...
    $counter++;
    $query="Przynaleznosc.G_ID_KIERUNEK='$przynaleznosc[0]' AND Przynaleznosc.G_ID_TYP='$przynaleznosc[1]' AND Przynaleznosc.G_ID_RODZAJ='$przynaleznosc[2]'";
...
$przynquery="SELECT Lista_Semestrow.ID_LISTA_SEMESTROW, Lista_Semestrow.L_LETNI, Lista_Semestrow.L_NR_SEMESTRU "
        ...
        . " WHERE Lista_Semestrow.L_LETNI='$_POST[semestrLZ]' AND Lista_Semestrow.L_ROK='$_POST[rok_akademicki]' AND Przynaleznosc.G_NUMER_ALBUMU <>'' AND ($query) AND ($LOplatystring) AND LRata.LR_DATA_PLATNOSCI<'$dzis'";
$iluquery="SELECT DISTINCT"
        ...
        . " WHERE Lista_Semestrow.L_LETNI='$_POST[semestrLZ]' AND Lista_Semestrow.L_ROK='$_POST[rok_akademicki]'  AND Przynaleznosc.G_NUMER_ALBUMU <>'' AND ($query)";
...
$czesne_oplacone_query="SELECT LinkOplaty.LL_ID_RATA,Lista_Semestrow.ID_LISTA_SEMESTROW, Lista_Semestrow.L_LETNI, Lista_Semestrow.L_NR_SEMESTRU "
        ...
        . "Lista_Semestrow INNER JOIN Przynaleznosc ON Lista_Semestrow.L_ID_Przynaleznosc=Przynaleznosc.ID_Przynaleznosc INNER JOIN LOplaty ON LOplaty.LO_ID_LISTA_SEMESTROW=Lista_Semestrow.ID_Lista_Semestrow INNER JOIN LRata ON LRata.LR_ID_LOPLATA=LOplaty.ID_LOPLATA  "
        . "INNER JOIN LinkOplaty ON LinkOplaty.LL_ID_RATA=LRata.ID_LRATA WHERE Lista_Semestrow.L_LETNI='$_POST[semestrLZ]' AND Lista_Semestrow.L_ROK='$_POST[rok_akademicki]' AND Przynaleznosc.G_NUMER_ALBUMU <>'' AND ($query)  AND ($LOplatystring) AND LRata.LR_STATUS=3 AND LinkOplaty.LL_DATA<=LRata.LR_DATA_PLATNOSCI";
...
$czesne_oplacone_czesciowo_query="SELECT Lista_Semestrow.ID_LISTA_SEMESTROW, Lista_Semestrow.L_LETNI, Lista_Semestrow.L_NR_SEMESTRU "
        . ",Lista_Semestrow.L_ROK, Przynaleznosc.G_NUMER_ALBUMU, LRata.LR_KWOTA, LRata.LR_STATUS,LRata.LR_DATA_PLATNOSCI,LRata.LR_DATA_ZMIANY_K,LOplaty.LO_RODZAJ,LinkOplaty.LL_DATA ,LinkOplaty.LL_KWOTA,LinkOplaty.LL_STATUS FROM "
        . "Lista_Semestrow INNER JOIN Przynaleznosc ON Lista_Semestrow.L_ID_Przynaleznosc=Przynaleznosc.ID_Przynaleznosc INNER JOIN LOplaty ON LOplaty.LO_ID_LISTA_SEMESTROW=Lista_Semestrow.ID_Lista_Semestrow INNER JOIN LRata ON LRata.LR_ID_LOPLATA=LOplaty.ID_LOPLATA  "
        . "INNER JOIN LinkOplaty ON LinkOplaty.LL_ID_RATA=LRata.ID_LRATA WHERE Lista_Semestrow.L_LETNI='$_POST[semestrLZ]' AND Lista_Semestrow.L_ROK='$_POST[rok_akademicki]' AND Przynaleznosc.G_NUMER_ALBUMU <>'' AND ($query)  AND ($LOplatystring) AND LRata.LR_STATUS=1";

$czesne_nieoplacone_query="SELECT Lista_Semestrow.ID_LISTA_SEMESTROW, Lista_Semestrow.L_LETNI, Lista_Semestrow.L_NR_SEMESTRU "
        . ",Lista_Semestrow.L_ROK, Przynaleznosc.G_NUMER_ALBUMU, LRata.LR_KWOTA, LRata.LR_STATUS,LRata.LR_DATA_PLATNOSCI,LRata.LR_DATA_ZMIANY_K,LOplaty.LO_RODZAJ FROM "
        . "Lista_Semestrow INNER JOIN Przynaleznosc ON Lista_Semestrow.L_ID_Przynaleznosc=Przynaleznosc.ID_Przynaleznosc INNER JOIN LOplaty ON LOplaty.LO_ID_LISTA_SEMESTROW=Lista_Semestrow.ID_Lista_Semestrow INNER JOIN LRata ON LRata.LR_ID_LOPLATA=LOplaty.ID_LOPLATA  "
        . " WHERE Lista_Semestrow.L_LETNI='$_POST[semestrLZ]' AND Lista_Semestrow.L_ROK='$_POST[rok_akademicki]' AND Przynaleznosc.G_NUMER_ALBUMU <>'' AND ($query) AND LRATA.LR_STATUS=0 AND ($LOplatystring) AND LRATA.LR_DATA_PLATNOSCI<'$dzis'";
$czesne_oplacone_ogolem_query="SELECT Lista_Semestrow.ID_LISTA_SEMESTROW, Lista_Semestrow.L_LETNI, Lista_Semestrow.L_NR_SEMESTRU "
        . ",Lista_Semestrow.L_ROK, Przynaleznosc.G_NUMER_ALBUMU, LRata.LR_KWOTA, LRata.LR_STATUS,LRata.LR_DATA_PLATNOSCI,LRata.LR_DATA_ZMIANY_K,LOplaty.LO_RODZAJ FROM "
        . "Lista_Semestrow INNER JOIN Przynaleznosc ON Lista_Semestrow.L_ID_Przynaleznosc=Przynaleznosc.ID_Przynaleznosc INNER JOIN LOplaty ON LOplaty.LO_ID_LISTA_SEMESTROW=Lista_Semestrow.ID_Lista_Semestrow INNER JOIN LRata ON LRata.LR_ID_LOPLATA=LOplaty.ID_LOPLATA  "
        . "INNER JOIN LinkOplaty ON LinkOplaty.LL_ID_RATA=LRata.ID_LRATA WHERE Lista_Semestrow.L_LETNI='$_POST[semestrLZ]' AND Lista_Semestrow.L_ROK='$_POST[rok_akademicki]' AND Przynaleznosc.G_NUMER_ALBUMU <>'' AND ($query)  AND ($LOplatystring) AND LRata.LR_STATUS=3 ";
...
else {$udzial_terminowych="Nie dotyczy";
$udzial_nieterminowych="Nie dotyczy";
$udzial_nieoplaconych="Nie dotyczy";
...
$udzial_terminowych_k=round(($suma_oplacone/$suma_naliczone)*100,2);
    ...
    $udzial_nieterminowych_k="Nie dotyczy";
    $udzial_nieoplaconych_k="Nie dotyczy";
    $udzial_terminowych_k="Nie dotyczy";
}
if($suma_naliczone!=0&&$sumawplat!=0){
$tablearray1[]="$przynaleznosc[3];;$przynaleznosc[4];;$przynaleznosc[5];;$przynaleznosc[6];;$LOplatydotabelki;;"
        . "$ile_rat_w_terminie;;$ile_rat_po_terminie;;$ile_rat_czesciowo;;$ile_rat_nieoplacone;;$udzial_nieoplaconych %;;"
        . "$udzial_nieterminowych %;;$udzial_terminowych %";

$tablearray2[]="$przynaleznosc[3];;$przynaleznosc[4];;$przynaleznosc[5];;$przynaleznosc[6];;$LOplatydotabelki;;"
        . "".number_format($suma_oplacone, 2, ',', ' ')." zł;;".number_format($suma_oplacone_po_terminie, 2, ',', ' ').""
        . " zł;;".number_format($suma_oplacone_czesciowo, 2, ',', ' ')." zł;;"
        . "".number_format($suma_nieoplacone, 2, ',', ' ')." zł;;$udzial_nieoplaconych_k;;"
        . "$udzial_nieterminowych_k %;;$udzial_terminowych_k %";
}
}
while($counter<$ile_p);