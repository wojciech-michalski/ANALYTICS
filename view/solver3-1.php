...
$values_query="SELECT * FROM `$tabela` WHERE `$element`=$values_query_element";
$values_query_negative="SELECT * FROM `$tabela` WHERE `$element`<>$values_query_element";
...
if ($condition=="LIKE") {
   ...
   // echo $wartosc;
    ...
    $values_query="SELECT * FROM `$tabela` WHERE `$element` LIKE $values_query_element";
 $values_query_negative="SELECT * FROM `$tabela` WHERE `$element` NOT LIKE $values_query_element";
...
if($element[-1]=='_') $element_=rtrim($element,"_");
//echo "SELECT * FROM `metryki` WHERE `ankieta`='$tabela' AND `pytanie`<>'$element_'";
//echo "SELECT `tresc` FROM `metryki` WHERE `ankieta`='$tabela'"
  //      . " AND `pytanie`='$element_'";

//echo $element_;
$pytania=mysqli_query($kon,"SELECT * FROM `metryki` WHERE `ankieta`='$tabela' AND `pytanie`<>'$element_'");
$tresc_elbad=mysqli_fetch_array(mysqli_query($kon,"SELECT `tresc` FROM `metryki` WHERE `ankieta`='$tabela'"
        . " AND `pytanie`='$element_'"));
...
do {
   ...
   switch($question[2]){
    ...
    $warianty=mysqli_query($kon,"SELECT DISTINCT `$pyt` FROM `TMP` WHERE 1 "); 
        $ile_wariantow=mysqli_num_rows($warianty);
        $k=0;
        do {
            $wariant=mysqli_fetch_array($warianty);
            $ile_wynikow=mysqli_num_rows(mysqli_query($kon,"SELECT `id` FROM `TMP` WHERE `$pyt`='$wariant[0]' "));
            ...
            if($rezultat_proc>=$treshold){
               ...
               $ile_wynikow_neg=mysqli_num_rows(mysqli_query($kon,"SELECT `id` FROM `TMP_NEGATIVE` WHERE `$pyt`<>'$wariant[0]'"));
            ...
            $result_matrix[]="$pyt;;;$question[3];;;$wariant[0];;;$rezultat_proc;;;$rezultat_proc_neg";
            }
            }
            else echo"";
            $k=$k+1;
...
<h5> Poszukiwana jest zależność pomiędzy pytaniem <?php echo "$element : <em>$tresc_elbad[0]</em>";?> <br/>
    Udzielone odpowiedzi: <?php echo konwersja_znakow($values_query_element);?>
    </b> - a innymi elementami ankiety</h5>

    <form method="POST" action="main.php?mode=deanreport15">
        <input type="hidden" name="analiza" value="ankieta">
        <button class="btn btn-unique" href="main.php?mode=deanreport15">Powrót</button>
    </form>
  ...
  <thead>
    <tr>
       ...
       //print_r($result_matrix);
        foreach($result_matrix as $result){
            $row=explode(";;;",$result);
           $r=mysqli_fetch_array(mysqli_query($kon,"SELECT `pytanie` FROM `metryki` WHERE `tresc`='$row[1]' "
                   . "AND `ankieta`='ankieta'"));
            //P VALUE:
                $ka=$ile_wynikow_query;
                $en=mysqli_fetch_array(mysqli_query($kon,"SELECT `id`,`typ`,`n` FROM"
                        . " `metryki` WHERE `pytanie`='$r[0]'"));
                switch($en[1]) {
                    case "radio":
                        $N=$en[2];
                        break;
    ...
    // _____________________________________________ _________________________________________________________________
                ...
                $pmian=((1-$corelation/100)*$ka);
                         echo "<tr><td>$row[0]</td><td>". konwersja_znakow($row[1])."<br/><strong><em>".konwersja_znakow($row[2])."</em></strong></td>"
